...
export default function StudyPlanner() {
  ...
  const toggleBlock = (dayIdx, blockIdx) => {
  ...
  const completedCount = currentDay.blocks.filter((_, i) => completedBlocks[`${selectedDay}-${i}`]).length;
  const progress = Math.round((completedCount / currentDay.blocks.length) * 100);

  const typeLabels = { tekrar: "Tekrar", hazirlik: "Hazırlık", okuma: "Okuma", pekistirme: "Pekiştirme", test: "Test" };
  const typeColors = { tekrar: "#3498DB", hazirlik: "#E67E22", okuma: "#27AE60", pekistirme: "#9B59B6", test: "#E74C3C" };
        ...
        <div style={{ padding: "16px" }}>
          ...
          <div style={{ display: "flex", gap: "6px", overflowX: "auto", padding: "4px 0 12px", WebkitOverflowScrolling: "touch" }}>
            {STUDY_PLAN.map((day, idx) => {
              const dayCompleted = day.blocks.every((_, i) => completedBlocks[`${idx}-${i}`]);
              const dayPartial = day.blocks.some((_, i) => completedBlocks[`${idx}-${i}`]);
              return (
                <button
                  key={idx}
          ...
          {showSchool && SCHOOL_SCHEDULE[currentDay.dayKey] && (
            <div style={{
              ...
              <div style={{ display: "flex", flexWrap: "wrap", gap: "6px" }}>
                {SCHOOL_SCHEDULE[currentDay.dayKey].map((s, i) => (
                  <span key={i} style={{
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
            {currentDay.blocks.map((block, idx) => {
              const isCompleted = completedBlocks[`${selectedDay}-${idx}`];
          ...
          <div style={{
            ...
            <p style={{ margin: 0, fontSize: "12px", color: "#888", fontWeight: 700 }}>
